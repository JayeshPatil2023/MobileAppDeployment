...
<#
.SYNOPSIS
    Builds a clone/push URL, embedding GITHUB_TOKEN when available for non-interactive git auth.
#>
function Get-GitHubRepositoryGitUrl {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:GITHUB_TOKEN)) {
        return "https://oauth2:$($env:GITHUB_TOKEN)@github.com/$Owner/$RepositoryName.git"
    }
...
function Invoke-GitFetchBranch {
    ...
    # --no-tags avoids pulling tags/refs that can collide on case-insensitive filesystems (Windows).
...
try {
    ...
    $sourceRepoUrl = "https://github.com/$SourceOwner/$SourceRepository"
    ...
    git clone $clientCloneUrl $workingDir
    if ($LASTEXITCODE -ne 0) { throw "Failed to clone client repository $clientRepoUrl" }

    Set-Location $workingDir

    # Case-insensitive filesystems (Windows) cannot store remote refs that differ only by casing.
    git config core.ignoreCase true

    Write-MergeProgress -Percent 35 -Message "Adding source repository remote" -Stage "add-remote"
    if (-not (git remote | Select-String -SimpleMatch 'source')) {
        git remote add source $sourceRepoUrl
        if ($LASTEXITCODE -ne 0) { throw "Failed to add source remote $sourceRepoUrl" }
    }
    ...
    $remoteClientBranch = git ls-remote --heads origin $ClientBranch
    ...
    git show-ref --verify --quiet "refs/heads/$ClientBranch" | Out-Null
